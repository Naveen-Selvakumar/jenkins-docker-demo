pipeline {
    ...
    environment {
        ...
        REGISTRY_CREDENTIALS = "naveen114"  // Jenkins docker credentials ID 2!#
    }
    ...
    stages {
        stage('Checkout Code') {
            steps {						
                withCredentials([usernamePassword(credentialsId: 'github-Naveen-Selvakumar', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_TOKEN')]) {  //3!# git credentials ID
                    git url: "https://$GIT_USER:$GIT_TOKEN@github.com/Naveen-Selvakumar/jenkins-docker-demo.git", branch: 'main' //4!#git user-name repository ID
                }
            }
        }
        ...
        stage('Login to Docker Registry') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'naveen114', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
